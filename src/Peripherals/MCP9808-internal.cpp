...
bool MCP9808::writeReg(uint8_t addr, uint16_t &data) {
...
bool MCP9808::readReg(uint8_t addr, uint16_t &result) {
...
bool MCP9808::setReg(uint8_t addr, uint16_t mask, uint16_t setting) {
    uint16_t previous;
    if (!readReg(addr, previous)) { // store the previous value of the register
        return false;
    }
    uint16_t newSetting = (mask & previous) | setting;
    if (!writeReg(addr, newSetting)) { // write the new one after changing the desired setting
        return false;
    }
    return true;
}
